...
works/001-004 すべて FTP 公開済み・本番稼働中。Search Console 設定は B-1〜B-3 完了、B-4 (リッチリザルト) ・B-5 (PageSpeed) を 2026-06-25 夜に着手。
...
1. **B-4 リッチリザルトテストの結果確認** — ユーザーが iPad でテスト中。Article + パンくず + Service が正しく検出されればゴール
2. **works/004 (および 001-003) のパフォーマンス改善** — LCP 9.4 秒は SEO 順位に悪影響。レンダーブロック解消が最大効果（7 秒短縮見込み）
   - Tailwind を CDN → ローカルビルドに切替
   - Google Fonts / Font Awesome の遅延読み込み
   - .htaccess のキャッシュ期間延長
3. **NOW.md 圧縮の commit & push**（このセッション中）
...
- **Search Console B-1〜B-3 完了**（サイトマップ・URL 検査 7 件・robots.txt 検証）
