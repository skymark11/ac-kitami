...
最終更新: 2026-06-25 深夜（PC セッション・Phase 0 ベースライン雛形 + git タグ完了）
...
1. **Phase 0 ベースライン記録**（ユーザー手動分）
   - [ ] Search Console: 主要 KW 順位・CTR・表示回数を `seo-audit/baseline-2026-06-25.md` の表に記入
   - [ ] PageSpeed Insights: トップ + works/* + about + abashiri/works/* をモバイル & PC で測定して同表に記入
   - [x] Git タグ `before-perf-2026-06-25` 作成 + push 完了（ロールバック点確保）
   - [x] `seo-audit/baseline-2026-06-25.md` 雛形作成済み
2. **Phase 1+2+3 一括実施**（ベースライン記入完了後・1 セッション完結）
   - Phase 1: `.htaccess` 強化（Gzip / mod_expires / セキュリティヘッダー / ETag 無効化 / KeepAlive）
   - Phase 2: works/* の Google Fonts 非同期化（トップ index.html と同方式へ統一）
   - Phase 3: works/001-004 FV B/A 画像を `<picture>` タグで WebP フォールバック化
   - 期待効果: LCP 9.4 秒 → 4-5 秒帯
3. **Phase 4+5（リスク中・効果次第で判断）**
...
### 2026-06-25 深夜（Phase 0 セッション）
- `seo-audit/baseline-2026-06-25.md` 雛形作成（Search Console KW 表 + PageSpeed 表 + Lighthouse 表）
- Git タグ `before-perf-2026-06-25` 作成 + origin に push（ロールバック点確保）
- 次セッション: ユーザーが手動で Phase 0 表を埋めたら Phase 1+2+3 に進む
...
- [ ] `seo-audit/baseline-2026-06-25.md` の Search Console KW 表 + PageSpeed 表が埋まっているか確認
- [ ] 埋まっていれば Phase 1+2+3 に着手（`.htaccess` 強化 + works/* Font 非同期化 + FV 画像 WebP 化）
- [ ] 改修後 PageSpeed 再測定 → ベースラインと比較記録（同ファイルに「改修後」セクション追記）
